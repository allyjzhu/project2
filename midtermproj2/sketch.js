...
const hOffsetsA = [1.5, 2, 2.5, 3, 3.5];
const normYValuesA = [0.95, 0.4, 0, 0.4, 0.95, 0.4];

const hOffsetsB = [2.5, 3, 2, 2, 3, 2.5];
const normYValuesB = [0.6, 0.85, 0, 1, 0.15, 0.4];

const hOffsetsC = [3.05, 2.25, 1.75, 1.75, 2.25, 3.05];
const normYValuesC = [1.0, 0.9, 0.6, 0.4, 0.1, 0];
...
const levelB = {
  id: 'B',
  hOffsets: hOffsetsB,
  normYValues: normYValuesB,
}
const levelC = {
  id: 'C',
  hOffsets: hOffsetsC,
  normYValues: normYValuesC,
}
//an array for our data and one for our levels
let levelData = [levelA, levelC, levelA, levelB];
var levels = Array.from({length: levelData.length});
let levelCounter = 0;
...
function setup() {
  ...
  setInterval(levelTime, 1000);
  //setInterval(initLevels(levels, 0), 1000);
  initLevels();
...
function draw() {
  ...
  myVida.setActiveZonesNormFillThreshold(0.00001);
  initLevels();
  ...
  //  myVida.setActiveZonesNormFillThreshold(0.99999);
    ...
    */
    ...
    console.log(myVida.thresholdImage)
    ...
    // let's iterate over all active zones
      ...
      //console.log(levels[levelCounter].zones[levelCounter]);
...
function forTimer(){
  if (timerValue >= 10){
    fill(255,255,255);
    text("0:" + timerValue, width - 80, height - 580);
   
  }
  if (timerValue < 10) {
    fill(255,255,255);
    text('0:0' + timerValue, width - 80, height - 580);
  }
  if (timerValue == 0) {
    timerValue = 10;
...
function initLevels(){
  ...
  if(levelTimer < 10){
  levels[0] = new Level(levelData[0])}
  else if(levelTimer > 10){
    //levels[0].removeZones(hOffsetsA);
    levels[1] = new Level(levelData[1])
  }
  else if(levelTimer > 20){
    //levels[1].removeZones(hOffsetsB);
    levels[2] = new Level(levelData[2])}
  else if(levelTimer > 30){

      levels[3] = new Level(levelData[3])
    }
  else if(levelTimer > 40){

      levels[4] = new Level(levelData[4])
    }

 }

/*function timerLevels(){
  var counter = 0;
  myVida.activeZones[counter] = levels[counter].zones[counter];
  counter++;
}
*/

/*function initLevels(levels, i){
  var i = 0;
 setInterval(
function(){
      levels[i] =  new Level(levelData[i]);
